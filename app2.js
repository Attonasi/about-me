...
var correctAnswer = 0;
var questionsAnswersArray = [['Does Collin have any cats?', 'no', 'n'], ['Does Collin have a job?', 'no', 'n'], ['Does Collin have any idea what he is doing?', 'no', 'n'], ['Is Collin going to get any sleep tonight?', 'no', 'n'], ['Does Collin like riding the train?', 'no', 'n']];
...
for(var k = 0; k < questionsAnswersArray; k++){
  response[k] = [prompt('Does Collin have any cats?').toLowerCase()];

  if (response[k] === 'yes' || response[k]=== 'y'){
    alert ('you are incorrect I dont have no stinking cats,');
  }else if(response[k] ==='no' || response[k] === 'n'){
