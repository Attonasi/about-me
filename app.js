...
alert('You are going to guess a random number between 1 and 20. You have 4 chances to get it right.')
...
for (var i=0; i<4; i++) {
  ...
  if (userGuess === correctGuessNumber) {
    alert('You guessed '+correctGuessNumber+' correctly on try number ' +(i+1)+'.');
    i=10;
    correctAnswer++;
  }else if (userGuess > correctGuessNumber){
    alert(userGuess +' is too high! That was guess number '+(i+1)+'.');
  }else {
    alert(userGuess +' is too low! That was guess number '+(i+1)+'.');
  }
  if (i===3){
